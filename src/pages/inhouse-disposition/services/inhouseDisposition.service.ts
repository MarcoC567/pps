import { DispositionInput, DispositionValues, PartBOM } from "../util/bom.ts";
...
export class DispositionService {
  ...
  public calculateDispositionValues(
    productBOMs: PartBOM[],
  ...
  private traverseAndCalculate(
    ...
    const productionOrder = this.calculateAndSetProdOrder(
      partBOM,
      input,
      parent,
      carryOver
    ...
    if (partBOM.parts) {
      partBOM.parts.forEach((part) => {
        this.traverseAndCalculate(
          ...
          input,
          partBOM.partId,
          input.get(part.partId)?.waitingQueue ?? 0
        );
      });
    }
  }

  private calculateAndSetProdOrder(
    partBOM: PartBOM,
    input: DispositionInput,
    parent: string,
    carryOver?: number
  ): number {
    const partId = partBOM.isUsedInAll
      ? partBOM.partId + "_" + parent
      : partBOM.partId;
    const productionOrder = this.calculateProductionOrder(
      partBOM,
      input,
      parent,
      carryOver
  ...
  private calculateProductionOrder(
    partBOM: PartBOM,
    dpInput: DispositionInput,
    parent: string,
    carryOver?: number
  ): number {
    const partId = ["E16", "E17", "E26"].includes(partBOM.partId)
      ? partBOM.partId + "_" + this.mapParent(parent)
      : partBOM.partId;
